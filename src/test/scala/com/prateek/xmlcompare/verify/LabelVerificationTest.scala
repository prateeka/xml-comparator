...
class LabelVerificationTest extends AnyFunSpec {

  describe("test LabelVerification") {

    it("test for matching nodes") {
      val exp = <Node>"test for matching node</Node>
      val act = <Node>"test for matching node</Node>
      val ctx: Context = Context().append(exp)
      assertResult(Match)(LabelVerification(exp, act)(using ctx))
    }
    ...
    it("test for non-matching nodes") {
      ...
      assertResult(NodeNotFound("Node1"))(
        LabelVerification(exp, act)(using ctx)
      )
    }
  }
}
